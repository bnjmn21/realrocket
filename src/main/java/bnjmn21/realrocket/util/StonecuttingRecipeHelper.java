    ...
    public static void toJson(JsonObject json, ResourceLocation id, Ingredient input, ItemStack output) {
        ...
        if (output.isEmpty()) {
            ...
            json.addProperty("result",
                    Objects.requireNonNull(ForgeRegistries.ITEMS.getKey(output.getItem())).toString());
            json.addProperty("count", output.getCount());
        }
    }

    public static void save(Consumer<FinishedRecipe> consumer, ResourceLocation id, Ingredient input,
                            ItemStack output) {
